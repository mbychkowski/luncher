...
description: >-
  ...
  using the product_manager agent persona, and save spec files with an auto-incremented index to /sdlc/01_specs/.
---

# Product Skill

The `product` skill initiates the SDLC process by running the `product_manager` agent interactively to build a feature spec with the user.
...
4. **Persist Spec**:
   ...
   - Confirm to the user that the spec is created and advise them that they can run the `implement` workflow next.
